...
| Pull-based `MosaicFrameSource` was reverted, then removed (S-1, #38) | **Confirmed by the maintainer:** it shipped in 1.7.0 as a reliability-spec deliverable (bounded memory), then was reverted (PR #29, with PR #28 restoring pipelined GPU batches) because generation was **30–45 % slower** than the batched `images(for:)` path | Throughput is a hard requirement. The dead type was removed in S-1 (#38); the design stays in git history. Any backpressure must be benchmarked against the batched path; prefer producer credits over per-frame pulls (§2.6). |
